...
COTTON_FLAG_OPTIONS=(
  ...
  "--verify-return:verify_return:1"
  "--require-upload:verify_return:1:require_upload:1"
...
capture_installed_package() {
  ...
  cotton_capture_text "05-package-dumpsys.txt" cotton_adb shell dumpsys package "$package_id"
...
write_metadata
write_checklist
cotton_capture_text "00-device.txt" cotton_adb shell getprop || true
ensure_device_ready
...
if [[ "$install_debug" -eq 1 ]]; then
  ...
  cotton_capture_text "03-install-debug.txt" cotton_install_android_apk "$serial" "$package_id" "$COTTON_ANDROID_APK"
fi

capture_installed_package
cotton_capture_text "08-play-services.txt" cotton_adb shell dumpsys package com.google.android.gms || true

cotton_adb logcat -c >/dev/null 2>&1 || true

if [[ "$launch_app" -eq 1 ]]; then
  cotton_capture_text "11-launch.txt" cotton_adb shell monkey -p "$package_id" 1
  sleep 2
fi

cotton_capture_screen "10-preflight"
...
prompt_capture "Open Cotton Files in the folder where the scanned PDF should upload. Leave Add files visible." \
  "20-files-ready"
cotton_require_xml_text "$evidence_dir/20-files-ready.xml" "Files" "Files screen title is missing."
cotton_require_xml_text "$evidence_dir/20-files-ready.xml" "Add files" "Files screen did not expose Add files."

prompt_capture "Tap Add files and leave the native Add action sheet visible." \
  "30-add-actions"
cotton_require_xml_text "$evidence_dir/30-add-actions.xml" "New folder" "Add sheet did not show New folder."
cotton_require_xml_text "$evidence_dir/30-add-actions.xml" "Upload..." "Add sheet did not show Upload..."
cotton_require_xml_text "$evidence_dir/30-add-actions.xml" "Scan document" "Add sheet did not show Scan document."
...
if [[ "$verify_return" -eq 1 ]]; then
  prompt_capture "Complete or cancel the scanner flow and return to Cotton Files." \
    "50-scan-return"
  cotton_require_xml_text "$evidence_dir/50-scan-return.xml" "Files" "Cotton Files was not visible after scanner return."
...
cotton_capture_text "90-logcat.txt" cotton_adb logcat -d -v threadtime || true
assert_no_fatal_logcat
