...
{
    public class DeviceToCloudSyncRootSetupServiceTests : IDisposable
    {
        ...
        private readonly CottonDeviceToCloudSyncRootSetupService _service;

        public DeviceToCloudSyncRootSetupServiceTests()
        {
            _directory = Path.Combine(
                Path.GetTempPath(),
                "cotton-device-to-cloud-root-setup-tests",
                Guid.NewGuid().ToString("N"));
            _rootStore = new FileSystemCottonSyncRootStore(new FixedSyncRootMetadataPathProvider(_directory));
            _service = new CottonDeviceToCloudSyncRootSetupService(_rootStore);
        }

        [Fact]
        public async Task Enable_user_selected_document_tree_root_creates_ready_device_to_cloud_root()
        {
            ...
            CottonDeviceToCloudSyncRootSetupResult result =
        ...
        public async Task Enable_user_selected_document_tree_root_is_idempotent_for_same_grant()
        {
            ...
            CottonDeviceToCloudSyncRootSetupResult first =
                await _service.EnableUserSelectedDocumentTreeRootAsync(
                    InstanceUri,
                    "account-1",
                    folder,
                    localRoot);
            CottonDeviceToCloudSyncRootSetupResult second =
                await _service.EnableUserSelectedDocumentTreeRootAsync(
                    InstanceUri,
                    "account-1",
                    folder,
        ...
        public async Task Enable_user_selected_document_tree_root_allows_different_folders_for_same_account()
        {
            ...
            Assert.All(roots, root => Assert.Equal(CottonSyncDirection.DeviceToCloud, root.Direction));
        }
        ...
        [Fact]
        public async Task Enable_user_selected_document_tree_root_reports_direction_conflict_without_replacing_root()
        {
            CottonUploadDestinationSnapshot folder = CreateFolder(FolderId, "Projects");
            CottonSyncLocalRootSnapshot localRoot = CreateDocumentTreeRoot("content://tree/primary%3AProjects", "Projects");
            CottonSyncRootSnapshot cloudToDeviceRoot = CreateRoot(
                Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"),
                folder,
                localRoot,
                CottonSyncDirection.CloudToDevice);
            await _rootStore.SaveAsync(InstanceUri, [cloudToDeviceRoot]);
            ...
            Assert.True(result.DirectionConflict);
            Assert.False(result.Enabled);
            Assert.Equal(cloudToDeviceRoot.Id, result.Root.Id);
            Assert.Equal(CottonSyncDirection.CloudToDevice, result.Root.Direction);
            CottonSyncRootSnapshot saved = Assert.Single(await _rootStore.LoadAsync(InstanceUri));
            Assert.Equal(cloudToDeviceRoot.Id, saved.Id);
            Assert.Equal(CottonSyncDirection.CloudToDevice, saved.Direction);
        }

        [Fact]
        public async Task Enable_user_selected_document_tree_root_treats_existing_bidirectional_root_as_configured()
        {
            CottonUploadDestinationSnapshot folder = CreateFolder(FolderId, "Projects");
            CottonSyncLocalRootSnapshot localRoot = CreateDocumentTreeRoot("content://tree/primary%3AProjects", "Projects");
            CottonSyncRootSnapshot bidirectionalRoot = CreateRoot(
                Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"),
                folder,
                localRoot,
                CottonSyncDirection.Bidirectional);
            await _rootStore.SaveAsync(InstanceUri, [bidirectionalRoot]);
            ...
            Assert.True(result.Enabled);
            Assert.Equal(bidirectionalRoot.Id, result.Root.Id);
