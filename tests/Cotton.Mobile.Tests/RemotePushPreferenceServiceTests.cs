...
{
    public class RemotePushPreferenceServiceTests
    {
        private static readonly Uri InstanceUri = new("https://cloud.example");

        [Fact]
        public async Task GetCurrentAsync_sends_authorized_get()
        {
            ...
            CottonRemotePushPreferences result = await service.GetCurrentAsync(InstanceUri);

            Assert.False(result.SharedFile);
            Assert.False(result.AccessRequest);
            Assert.False(result.CommentMention);
            Assert.True(result.SecuritySession);
            Assert.Equal(1, result.EnabledCategoryCount);

            RecordedRequest request = Assert.Single(handler.Requests);
            Assert.Equal(HttpMethod.Get, request.Method);
            Assert.Equal("/api/v1/users/me/preferences/push-notifications", request.Uri.PathAndQuery);
        ...
        [Fact]
        public async Task UpdateCurrentAsync_sends_authorized_patch_payload()
        {
            var handler = new RecordingHttpMessageHandler();
            handler.EnqueueJson(HttpStatusCode.OK, new
            {
            ...
            CottonRemotePushPreferences result = await service.UpdateCurrentAsync(
                InstanceUri,
            ...
            Assert.True(result.SharedFile);
            Assert.False(result.AccessRequest);
            Assert.True(result.CommentMention);
            Assert.True(result.SecuritySession);
            Assert.Equal(3, result.EnabledCategoryCount);
            ...
            Assert.True(root.GetProperty("sharedFile").GetBoolean());
            Assert.False(root.GetProperty("accessRequest").GetBoolean());
            Assert.True(root.GetProperty("commentMention").GetBoolean());
            Assert.True(root.GetProperty("securitySession").GetBoolean());
        }

        [Fact]
        public async Task UpdateCurrentAsync_refreshes_token_once_after_unauthorized_response()
        {
            var handler = new RecordingHttpMessageHandler();
            ...
            await service.UpdateCurrentAsync(
                InstanceUri,
                CottonRemotePushPreferences.Default.WithCategory(
            ...
            Assert.Equal("/api/v1/users/me/preferences/push-notifications", handler.Requests[2].Uri.PathAndQuery);
        }
        ...
        private static CottonRemotePushPreferenceService CreateService(
            ...
            ICottonTokenStore tokenStore)
        {
            ...
            return new CottonRemotePushPreferenceService(apiClient);
        }
