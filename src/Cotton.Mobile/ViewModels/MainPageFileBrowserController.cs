...
namespace Cotton.Mobile.ViewModels
{
    public class MainPageFileBrowserController
    {
        ...
        private const string MoveToTrashAction = CottonFileTrashStatusText.ConfirmAction;
        ...
        public Task ClearSelectionAsync()
        {
            if (_display.IsFileSelectionActive)
            {
                SuppressEntryInteractionAfterSelectionDismissal();
            }
        ...
        private void SuppressEntryInteractionAfterSelectionDismissal()
        {
        ...
        private async Task ShowFileActionsAsync(CottonFileBrowserEntry file)
        {
            ...
            switch (action)
            {
                ...
                case VersionHistoryAction:
                    ...
                    break;
                case MoveToTrashAction:
                    await MoveFileToTrashAsync(currentFile, instanceUri);
                    break;
                case DetailsAction:
        ...
        private async Task DownloadSelectionFilesAsync(IReadOnlyList<CottonFileBrowserEntry> entries)
        {
            ...
            {
                _display.ShowFilesStatus(CottonFileBulkDownloadStatusText.UnavailableStatus);
                return;
            }

            if (selectedFiles.Length == 1)
            {
                await DownloadFileAsync(selectedFiles[0]);
                return;
            }

            if (!_networkAccess.HasInternetAccess)
            {
                _display.ShowFilesStatus(CottonFileBulkDownloadStatusText.OfflineUnavailableStatus);
                return;
            }

            CancellationTokenSource fileActionCancellation = BeginFileAction(
                CottonFileBulkDownloadStatusText.CreateStartingStatus(selectedFiles.Length));
            ...
            try
            {
                for (int index = 0; index < selectedFiles.Length; index++)
                {
                    ...
                    _display.ShowFileActionLoading(
                        CottonFileBulkDownloadStatusText.CreateDownloadingItemStatus(
                            index + 1,
                    ...
                    await _fileBrowserService.DownloadAsync(
                        instanceUri,
                        file,
                        CreateBulkFileDownloadProgress(
                            file,
                            index + 1,
                            selectedFiles.Length,
                            () => IsActiveFileAction(fileActionCancellation, instanceUri),
                ...
                await RefreshLocalFileStateAsync(instanceUri, CancellationToken.None);
                _display.ShowFilesStatus(CottonFileBulkDownloadStatusText.CreateCompletedStatus(downloadedCount));
            }
            catch (Exception exception)
                when (IsAuthorizationFailure(exception))
            {
                if (!IsActiveFileAction(fileActionCancellation, instanceUri))
                {
                    _logger.LogDebug(exception, "Ignored stale Cotton mobile selection download authorization failure.");
                    return;
                }
            ...
            catch (OperationCanceledException) when (fileActionCancellation.IsCancellationRequested)
            {
                if (!IsActiveFileAction(fileActionCancellation, instanceUri))
                {
                    _logger.LogDebug("Ignored stale Cotton mobile selection download cancellation.");
                    return;
                }
                ...
                _display.ShowFilesStatus(
                    CottonFileBulkDownloadStatusText.CreateCancelledStatus(
                        downloadedCount,
                        selectedFiles.Length));
            }
            catch (Exception exception)
            {
                if (!IsActiveFileAction(fileActionCancellation, instanceUri))
                {
                    _logger.LogDebug(exception, "Ignored stale Cotton mobile selection download failure.");
                    return;
                }
                ...
                _logger.LogError(exception, "Failed to download Cotton mobile selection.");
                ClearFileActionRetry();
                _display.ShowFilesStatus(
                    CreateFileActionFailureStatus(
                        exception,
                        CottonFileBulkDownloadStatusText.CreateFailedStatus(
                            downloadedCount,
                            selectedFiles.Length),
                        CottonFileBulkDownloadStatusText.OfflineUnavailableStatus));
            }
            finally
            {
                EndFileAction(fileActionCancellation, shouldRunRecoveryRefresh: false);
        ...
        private async Task KeepSelectedEntryOfflineAsync(CottonFileBrowserEntry entry)
        {
            if (entry.Type == CottonFileBrowserEntryType.Folder)
            {
                await PlanFolderOfflineAsync(entry);
                return;
            }

            await KeepFileOfflineAsync(entry);
        }
        ...
        private async Task KeepSelectionOfflineAsync(IReadOnlyList<CottonFileBrowserEntry> entries)
        {
            ...
            if (instanceUri is null)
            {
                _display.ShowFilesStatus("Sign in to keep selected items offline.");
                return;
            }
            ...
            {
                _display.ShowFilesStatus(CottonFolderBulkOfflineStatusText.UnavailableStatus);
                return;
            }
            ...
            IReadOnlyList<CottonFileBrowserEntry> selectedFolders)
        {
            ...
            {
                await PlanFolderOfflineAsync(selectedFolders[0]);
                return;
            }

            _display.ShowFilesStatus(CottonFolderBulkOfflineStatusText.CreateStartingStatus(selectedFolders.Count));
            for (int index = 0; index < selectedFolders.Count; index++)
            {
                if (!CanUseFileBrowserContext(instanceUri))
                {
                    return;
                ...
                CottonFileBrowserEntry? currentFolder =
                    GetCurrentVisibleEntry(selectedFolders[index], instanceUri);
                if (currentFolder is null || currentFolder.Type != CottonFileBrowserEntryType.Folder)
                {
                    _display.ShowFilesStatus(CottonFolderBulkOfflineStatusText.UnavailableStatus);
                    return;
                }
                ...
                _display.ShowFilesStatus(
                    CottonFolderBulkOfflineStatusText.CreateCheckingFolderStatus(
                        index + 1,
                        selectedFolders.Count,
                        currentFolder.Name));
                bool shouldContinue = await PlanFolderOfflineAsync(currentFolder);
                if (!shouldContinue)
                {
                    return;
                }
            ...
            IReadOnlyList<CottonFileBrowserEntry> selectedFiles)
        {
            ...
            CancellationTokenSource fileActionCancellation = BeginFileAction(
                CottonFileBulkOfflineStatusText.CreateStartingStatus(selectedFiles.Count));
        ...
        private async Task<bool> PlanFolderOfflineAsync(CottonFileBrowserEntry folder)
        {
            if (!folder.IsFolder)
            {
                return false;
        ...
        private async Task MoveFileToTrashAsync(CottonFileBrowserEntry file)
        {
            Uri? instanceUri = _instanceUri;
            if (instanceUri is null)
            {
                _display.ShowFilesStatus("Sign in to move files to trash.");
                return;
            }
        ...
        private async Task MoveFileToTrashAsync(CottonFileBrowserEntry file, Uri instanceUri)
        {
            ...
            {
                ClearFileActionRetry();
                _display.ShowFilesStatus(CottonFileTrashStatusText.NeedsRefreshStatus);
                return;
            }

            bool confirmed = await _dialogService.ShowConfirmationAsync(
                CottonFileTrashStatusText.ConfirmTitle,
                CottonFileTrashStatusText.CreateConfirmMessage(file.Name),
                CottonFileTrashStatusText.ConfirmAction,
                CancelAction);
            ...
            CancellationTokenSource fileActionCancellation = BeginFileAction(
                CottonFileTrashStatusText.CreateMovingStatus(currentFile.Name));
            bool didEndFileAction = false;
                ...
                when (IsAuthorizationFailure(exception))
            {
                if (!IsActiveFileAction(fileActionCancellation, instanceUri))
                {
                    _logger.LogDebug(exception, "Ignored stale Cotton mobile trash authorization failure {FileId}.", file.Id);
                    return;
                }
            ...
            catch (Exception exception)
            {
                if (!IsActiveFileAction(fileActionCancellation, instanceUri))
                {
                    _logger.LogDebug(exception, "Ignored stale Cotton mobile trash failure {FileId}.", file.Id);
                    return;
                }

                _logger.LogWarning(exception, "Failed to move Cotton mobile file to trash {FileId}.", file.Id);
                ShowFileActionRetry(
                    MainPageFileAction.MoveToTrash,
                    file,
                    CreateFileActionFailureStatus(
                        exception,
                        CottonFileTrashStatusText.FailedStatus,
                        CottonFileTrashStatusText.OfflineUnavailableStatus));
            }
            finally
            {
        ...
        private async Task ClearTrashedFileLocalStateBestEffortAsync(Uri instanceUri, CottonFileBrowserEntry file)
        {
            ...
            {
                await RemoveFileOfflinePinAsync(instanceUri, file.Id, CancellationToken.None);
                await _fileBrowserService.DeleteLocalDownloadAsync(instanceUri, file, CancellationToken.None);
            }
            catch (Exception exception)
            {
                _logger.LogWarning(exception, "Failed to clear Cotton mobile trashed file local state {FileId}.", file.Id);
            }
        }
        ...
        private IProgress<long>? CreateBulkFileDownloadProgress(
            CottonFileBrowserEntry file,
            int itemPosition,
            int fileCount,
            Func<bool> canUpdateProgress,
